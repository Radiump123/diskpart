...
- RAID/virtual-disk equivalents: `add`, `break`, `attach vdisk`, `detach vdisk`, `create vdisk`, `compact`, `merge vdisk`, `import`
- diagnostics/policy: `dump`, `recover`, `repair`, `retain`, `san [policy=onlineall|offlineall]`
...
All standard DiskPart command names are recognized in the parser; commands without a Linux-native implementation return an explicit "recognized but not implemented" response instead of an unknown-command error.
