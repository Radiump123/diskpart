...
static exit_code cmd_dump(void)
{
    puts("# diskpart Linux session dump");
    printf("# selected_disk=%s\n", g_state.selected_disk[0] ? g_state.selected_disk : "(none)");
    printf("# selected_partition=%s\n", g_state.selected_partition[0] ? g_state.selected_partition : "(none)");
    printf("# selected_volume=%s\n", g_state.selected_volume[0] ? g_state.selected_volume : "(none)");
    printf("# san_policy=%s\n", g_state.san_policy[0] ? g_state.san_policy : "onlineall");

    if (g_state.selected_disk[0])
        run_cmd("lsblk -o NAME,SIZE,TYPE,FSTYPE,MOUNTPOINTS %s", g_state.selected_disk);
    else
        run_cmd("lsblk -d -o NAME,SIZE,TYPE,MODEL");

    return EXIT_OK;
}

static exit_code cmd_retain(void)
{
    char disk[MAX_PATH];
    int partnum;
    ...
    /* Approximate RETAIN semantics on GPT: set "do not automount" attr bit 63. */
    return run_cmd("sgdisk --attributes=%d:set:63 %s", partnum, disk) == 0 ? EXIT_OK : EXIT_SERVICE;
}

static exit_code cmd_san(int argc, char *argv[])
{
    ...
    if (strcasecmp(policy, "onlineall") != 0 && strcasecmp(policy, "offlineall") != 0)
    {
        fputs("SAN policy must be onlineall or offlineall\n", stderr);
        return EXIT_SYNTAX;
    }

    snprintf(g_state.san_policy, sizeof(g_state.san_policy), "%s", policy);

    if (!require_root())
        return EXIT_SERVICE;

    if (!strcasecmp(policy, "offlineall"))
        return run_cmd("for d in $(lsblk -dn -o NAME,TYPE | awk '$2==\"disk\"{print $1}'); do blockdev --setro /dev/$d 2>/dev/null || true; done") == 0 ? EXIT_OK : EXIT_SERVICE;

    return run_cmd("for d in $(lsblk -dn -o NAME,TYPE | awk '$2==\"disk\"{print $1}'); do blockdev --setrw /dev/$d 2>/dev/null || true; done") == 0 ? EXIT_OK : EXIT_SERVICE;
}
...
static exit_code run_command(char *line)
{
    ...
    if (!strcasecmp(argv[0], "dump")) return cmd_dump();
    ...
    if (!strcasecmp(argv[0], "retain")) return cmd_retain();
    if (!strcasecmp(argv[0], "san")) return cmd_san(argc, argv);
...
int main(int argc, char **argv)
{
    ...
    snprintf(g_state.san_policy, sizeof(g_state.san_policy), "onlineall");
