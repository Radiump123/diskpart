...
static int require_root(void)
{
    ...
    fputs("This command requires root privileges (try sudo).\n", stderr);
    return 0;
}
...
static int resolve_indexed_device(const char *kind, const char *value, char out[MAX_PATH])
{
    char cmd[256];
    FILE *fp;
    char name[128];
...
{
    puts("Implemented commands on Linux:");
    puts("  select disk <N|/dev/...>");
    puts("  select partition <N|/dev/...>");
    puts("  select volume <N|/dev/...>");
    puts("  list disk | list partition | list volume");
    puts("  detail disk | detail partition | detail volume");
    puts("  active | inactive");
    puts("  add | break                    (mdadm mirror ops)");
    puts("  assign [mount=<path>] | remove [mount=<path>]");
    ...
    puts("  compact | merge vdisk | create vdisk");
    puts("  convert gpt | convert mbr");
    puts("  create partition primary|efi|msr [size=<MiB>] [start=<MiB>]");
    puts("  delete partition [override] | delete volume");
    puts("  extend [size=<MiB>] | shrink size=<MiB>");
    puts("  format [fs=ext4|xfs|vfat|exfat|ntfs] [label=<name>]");
    puts("  gpt attributes=<hex>");
    puts("  import | offline disk | online disk | rescan");
    puts("  set id=<GUID>");
    puts("  uniqueid disk [id=<GUID>]");
    puts("  rem ... | # ... | help | exit");
}

static int get_partnum_and_disk(const char *part_dev, char *disk, size_t dsz, int *partnum)
{
    char cmd[512];
    FILE *fp;
    char dname[128], pnum[32];
    ...
    if (fscanf(fp, "%127s %31s", dname, pnum) != 2)
    {
        pclose(fp);
        return 0;
    }
...
static exit_code cmd_select(int argc, char *argv[])
{
    ...
    {
        if (!resolve_indexed_device(argv[1], argv[2], dev))
        {
            fprintf(stderr, "No %s at index %s\n", argv[1], argv[2]);
            return EXIT_FILE;
        }
    }
    else
    ...
    if (!strcasecmp(argv[0], "partition"))
    {
...
static exit_code cmd_detail(int argc, char *argv[])
{
    ...
    if (!strcasecmp(argv[1], "disk"))
    {
        if (!g_state.selected_disk[0])
            return EXIT_SYNTAX;
        return run_cmd("lsblk -O %s && blkid %s", g_state.selected_disk, g_state.selected_disk) == 0 ? EXIT_OK : EXIT_SERVICE;
    }
    ...
    if (!strcasecmp(argv[0], "partition"))
    {
        if (!g_state.selected_partition[0])
            return EXIT_SYNTAX;
        return run_cmd("lsblk -O %s && blkid %s", g_state.selected_partition, g_state.selected_partition) == 0 ? EXIT_OK : EXIT_SERVICE;
    }
    ...
    if (!strcasecmp(argv[0], "volume"))
    {
        if (!g_state.selected_volume[0])
            return EXIT_SYNTAX;
        return run_cmd("lsblk -O %s && blkid %s", g_state.selected_volume, g_state.selected_volume) == 0 ? EXIT_OK : EXIT_SERVICE;
    }

    return EXIT_SYNTAX;
}

static exit_code cmd_active_common(int active)
{
    char disk[MAX_PATH];
    int partnum;
    ...
    if (!require_root())
        return EXIT_SERVICE;
    if (!get_partnum_and_disk(g_state.selected_partition, disk, sizeof(disk), &partnum))
        return EXIT_SERVICE;

    return run_cmd("parted -s %s set %d boot %s", disk, partnum, active ? "on" : "off") == 0 ? EXIT_OK : EXIT_SERVICE;
}

static exit_code cmd_assign(int argc, char *argv[])
{
    const char *mnt = arg_value(argc, argv, "mount");
    ...
    run_cmd("mkdir -p '%s'", mnt);
    return run_cmd("mount %s '%s'", g_state.selected_volume, mnt) == 0 ? EXIT_OK : EXIT_SERVICE;
}
...
static exit_code cmd_remove(int argc, char *argv[])
{
    ...
    if (mnt)
        return run_cmd("umount '%s'", mnt) == 0 ? EXIT_OK : EXIT_SERVICE;
...
static exit_code cmd_attributes(int argc, char *argv[])
{
    ...
    else if (!strcasecmp(argv[0], "volume"))
...
static exit_code cmd_clean(int argc, char *argv[])
{
    ...
    if (argc >= 1 && !strcasecmp(argv[0], "all"))
    {
...
static exit_code cmd_create(int argc, char *argv[])
{
    ...
    const char *start = arg_value(argc, argv, "start");

    if (argc < 2)
        return EXIT_SYNTAX;
    if (!require_root() || !g_state.selected_disk[0])
        return EXIT_SERVICE;

    if (!strcasecmp(argv[0], "partition") && !strcasecmp(argv[1], "primary"))
    {
        const char *s = start ? start : "1";
        char end[64];
        if (size)
            snprintf(end, sizeof(end), "%sMiB", size);
        else
    ...
    if (!strcasecmp(argv[0], "partition") && !strcasecmp(argv[1], "efi"))
    {
        const char *s = start ? start : "1";
        const char *sz = size ? size : "100";
        char end[32];
        snprintf(end, sizeof(end), "%d", atoi(s) + atoi(sz));
        if (run_cmd("parted -s %s mkpart ESP fat32 %sMiB %sMiB", g_state.selected_disk, s, end) != 0)
            return EXIT_SERVICE;
        run_cmd("parted -s %s set 1 esp on", g_state.selected_disk);
        return EXIT_OK;
    }

    if (!strcasecmp(argv[0], "partition") && !strcasecmp(argv[1], "msr"))
    {
        const char *s = start ? start : "101";
        char end[32];
        snprintf(end, sizeof(end), "%d", atoi(s) + (size ? atoi(size) : 16));
        return run_cmd("parted -s %s mkpart msr %sMiB %sMiB", g_state.selected_disk, s, end) == 0 ? EXIT_OK : EXIT_SERVICE;
    }

    if (!strcasecmp(argv[0], "vdisk"))
    {
...
static exit_code cmd_delete(int argc, char *argv[])
{
    ...
    if (!strcasecmp(argv[0], "volume"))
    {
        if (!g_state.selected_volume[0])
            return EXIT_SYNTAX;
        if (!get_partnum_and_disk(g_state.selected_volume, disk, sizeof(disk), &partnum))
            return EXIT_SERVICE;
        return run_cmd("parted -s %s rm %d", disk, partnum) == 0 ? EXIT_OK : EXIT_SERVICE;
    }
...
static exit_code cmd_extend(int argc, char *argv[])
{
    ...
    if (!g_state.selected_partition[0] || !require_root())
        return EXIT_SERVICE;
    if (!get_partnum_and_disk(g_state.selected_partition, disk, sizeof(disk), &partnum))
        return EXIT_SERVICE;
...
{
    const char *attrs = arg_value(argc, argv, "attributes");
    char disk[MAX_PATH];
    int partnum;
    ...
    return run_cmd("sgdisk --attributes=%d:set:%s %s", partnum, attrs, disk) == 0 ? EXIT_OK : EXIT_SERVICE;
}

static exit_code cmd_set(int argc, char *argv[])
{
...
static exit_code cmd_uniqueid(int argc, char *argv[])
{
    ...
    if (argc < 1 || strcasecmp(argv[0], "disk") != 0 || !g_state.selected_disk[0])
        return EXIT_SYNTAX;
...
static exit_code cmd_rescan(void)
{
    ...
    return run_cmd("udevadm settle || true") == 0 ? EXIT_OK : EXIT_SERVICE;
}

static exit_code cmd_create_merge_compact(int argc, char *argv[])
{
    if (argc < 2)
        return EXIT_SYNTAX;
    ...
    if (!strcasecmp(argv[0], "merge") && !strcasecmp(argv[1], "vdisk"))
    {
        const char *file = arg_value(argc, argv, "file");
        if (!file)
            return EXIT_SYNTAX;
...
static exit_code cmd_add_break(int argc, char *argv[])
{
...
static exit_code run_command(char *line)
{
    ...
    if (!strcasecmp(argv[0], "active")) return cmd_active_common(1);
    if (!strcasecmp(argv[0], "inactive")) return cmd_active_common(0);
    ...
    if (!strcasecmp(argv[0], "clean")) return cmd_clean(argc - 1, argv + 1);
    if (!strcasecmp(argv[0], "convert")) return cmd_convert(argc - 1, argv + 1);
    if (!strcasecmp(argv[0], "create")) return cmd_create(argc - 1, argv + 1);
    if (!strcasecmp(argv[0], "delete")) return cmd_delete(argc - 1, argv + 1);
    if (!strcasecmp(argv[0], "extend")) return cmd_extend(argc - 1, argv + 1);
    if (!strcasecmp(argv[0], "shrink")) return cmd_shrink(argc - 1, argv + 1);
    if (!strcasecmp(argv[0], "format")) return cmd_format(argc - 1, argv + 1);
    if (!strcasecmp(argv[0], "gpt")) return cmd_gpt(argc - 1, argv + 1);
    if (!strcasecmp(argv[0], "set") || !strcasecmp(argv[0], "setid")) return cmd_set(argc - 1, argv + 1);
    if (!strcasecmp(argv[0], "uniqueid")) return cmd_uniqueid(argc - 1, argv + 1);
    ...
    if (!strcasecmp(argv[0], "compact") || !strcasecmp(argv[0], "merge")) return cmd_create_merge_compact(argc, argv);
    if (!strcasecmp(argv[0], "add") || !strcasecmp(argv[0], "break")) return cmd_add_break(argc, argv);

    if (!strcasecmp(argv[0], "import"))
    {
        puts("import: dynamic foreign disk import is handled by LVM/mdadm tools on Linux.");
        return EXIT_OK;
    }

    fprintf(stderr, "Unknown or unsupported command syntax: %s\n", argv[0]);
    return EXIT_SYNTAX;
}
